...
import com.leilei.service.IClassService;
import com.leilei.entity.Class;
...
/**
 * <p>
 *  class前端控制器  RestController注解 将结果以JSON形式返回
 * </p>
 *
 * @author ${leilei}
 * @since 2020-08-02
...
public class ClassController {
    ...
    /**
     * 保存修改公用 POST请求方式
     * @param classa 修改或保存的对象
     * @return JsonReturn
     */
    @PostMapping("/save")
    public JsonReturn save(Class classa) {
        if (classa.getId() != null){
            try {
                classService.updateById(classa);
                return JsonReturn.buildSuccess(classa, "操作成功");
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public JsonReturn findOne(Long id) {
        try {
            Class classa = classService.getById(id);
            return JsonReturn.buildSuccess(classa, "操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            return JsonReturn.buildFailure("操作失败" + e.getMessage());
    ...
    public JsonReturn findAll(Integer pageNum, Integer pageSize) {
        ...
        if (pageNum != null && pageSize != null) {
            try {
                Page<Class> page = classService.page(new Page<Class>(pageNum, pageSize));
        ...
        try {
            Page<Class> page = classService.page(new Page<>());
            return JsonReturn.buildSuccess(page, "操作成功");
        } catch (Exception e) {
            e.printStackTrace();
            return JsonReturn.buildFailure("操作失败" + e.getMessage());
